...
def menu_list(request, path, bases):
    ...
    previous = int(request.GET.get('previous', 0) or 0)
    weekdays = [d for d in WEEKDAYS if d['day'] == default_day]
    selected_day = request.GET.get('day', weekdays[0]['day'] if weekdays else None)
    start_of_week = (today - timedelta(days=today_idx) if today_idx < 5 else today + timedelta(days=7-today_idx)) - timedelta(weeks=previous)
